...
import TestNoticeBanner from '../components/TestNoticeBanner'
...
export default function GrowthPlatformAgreement() {
  ...
  useEffect(() => {
    const saved = sessionStorage.getItem('gp_tl_customer_info')
    if (saved) {
      try {
  ...
  const handleSubmit = async (e) => {
    ...
    let contactId = ''
    try {
      const prior = JSON.parse(sessionStorage.getItem('gp_tl_customer_info') || '{}')
    ...
    sessionStorage.setItem('gp_tl_customer_info', JSON.stringify(sessionData))

    router.push('/funnels/growth-platform-test-live/checkout')
  }
      ...
      <div className="section-container-txai animate-fade-up-txai" style={{ marginTop: '40px' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ 
            ...
            <Link href="/funnels/growth-platform-test-live/agreement/msa" target="_blank" style={{ 
