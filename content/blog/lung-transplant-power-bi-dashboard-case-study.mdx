...
excerpt: "How Texas AI Consulting joined TeleResults SQL data with HCA HIN data accessed through BigQuery to build a lung transplant Power BI dashboard."
...
Texas AI Consulting built a lung transplant surveillance dashboard that brings direct transplant data from TeleResults together with radiology data available through the HCA Healthcare Intelligence Net (HIN) and queried through BigQuery. The result is one governed Power BI application with an exception-based view of the post-transplant population, longitudinal lab trends, protocol status, and the information needed for a daily clinical huddle.
...
- **The HCA Healthcare Intelligence Net (HIN)** served as the enterprise data and analytics environment that integrates information from clinical source systems for governed reporting and decision support.
...
<div className="my-8 overflow-x-auto rounded-lg border border-[#2c75ff]">
  <table className="w-full border-collapse text-left">
    ...
    <tbody>
      <tr>
        <td>Direct transplant source</td>
        <td>TeleResults backed by SQL Server</td>
        <td>Direct access to transplant dates, clinic activity, labs, pulmonary function, and patient-level surveillance fields</td>
      </tr>
      <tr>
        <td>Enterprise data and analytics environment</td>
        <td>HCA Healthcare Intelligence Net (HIN)</td>
        <td>Integrates information from clinical source systems for governed analytics, data lineage, historical reporting, and decision support</td>
      </tr>
      <tr>
        <td>Cloud access and query layer</td>
        <td>HCA Google Cloud Platform and BigQuery</td>
        <td>Governed query path into HIN used to locate radiology and other protocol data missing from the TeleResults SQL source</td>
      </tr>
      <tr>
...
That is where understanding HCA's broader data architecture mattered. HIN is an enterprise data and analytics environment that integrates information from clinical source systems for governed reporting and decision support. It is not another transactional EHR.

For this project, TeleResults data was accessed directly from its SQL Server backend, while radiology data missing from that source was located in HIN and queried through HCA's Google Cloud and BigQuery environment. This distinction kept the data lineage clear: TeleResults was the direct transplant source, HIN was the enterprise analytics environment, and BigQuery was the query technology used to reach the required HIN data.
...
The operational result was a single governed surveillance view spanning the direct TeleResults database and HIN data queried through BigQuery. The team could review the post-transplant panel through exceptions, trends, and protocol status instead of reconstructing the same picture one chart at a time.
