...
export const getBlogPostBySlug = cache(async (slug) => {
  try {
    ...
    console.log(`Trying .md file: ${fullPath}, exists: ${fs.existsSync(fullPath)}`);
    ...
    if (!fs.existsSync(fullPath)) {
      ...
      console.log(`Trying .mdx file: ${fullPath}, exists: ${fs.existsSync(fullPath)}`);
      
      // If neither exists, return null
      if (!fs.existsSync(fullPath)) {
        console.log(`Neither .md nor .mdx file exists for slug: ${slug}`);
        return null;
      }
    }
    
    console.log(`Reading file: ${fullPath}, isMDX: ${isMDX}`);
    const fileContents = fs.readFileSync(fullPath, 'utf8');
    console.log(`File content length: ${fileContents.length}`);
    console.log(`File content preview: ${fileContents.substring(0, 100)}...`);
    
    const { data, content } = matter(fileContents);
    console.log(`After matter parsing - content length: ${content.length}`);
